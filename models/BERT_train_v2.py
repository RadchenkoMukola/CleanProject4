...
train_encodings = stepwise_tokenize_with_cache(train_texts, tokenizer, max_length, "train_cache.pkl")
val_encodings = stepwise_tokenize_with_cache(val_texts, tokenizer, max_length, "val_cache.pkl")
...
rng_file = "results_bert_frozen_weighted/checkpoint-21555/rng_state.pth"
if os.path.exists(rng_file):
    os.remove(rng_file)

model = BertForSequenceClassification.from_pretrained("results_bert_frozen_weighted/checkpoint-21555", num_labels=3)
...
class WeightedTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):  # <-- додано параметр
...
gc.collect()
#trainer.train()
trainer.train(resume_from_checkpoint="results_bert_frozen_weighted/checkpoint-21555")